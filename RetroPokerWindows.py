...
print(' ')
print('Starting Instructions: The game launches after the IP address and the username are entered.')
print('8 players at a time can join a game. One player should hold off from pressing the buttons until')
print('all the other players have joined. The game starts after all players have pressed a button')
print('to acknowledge they are ready to start.')
print('New players cannot join after the game is launched. The server resets after 5 minutes of')
print('inactivity. In the event the game needs to be restarted, close clients and just wait for 5mins.')
print('Entering the code "9876" in the betting amount box puts that player in "Auto-Fold"')
print('mode. This is usefull if one of the players needs to take a break without disrupting')
print('the game flow for the other players.')
print('Maximum bet and raise is set to $1000. Each player starts with $10,000.')
print('Game ends after either preset amount of time or when 1st player goes bust.')
...
print('address prompt below. Just press enter to use the default poker server')
print(" ")
...
if len(IP)==0: #if playing multiple games or testing, it's useful to avoid multiple entry of IP address
    ...
    IP = '127.0.0.1' #"52.175.53.52" #"127.0.0.1"
...
class Goui:
    def __init__(self,root,queue,socket,display):
        ...
        invislabel2 = Label(bottomFrame, width =40, height=1) #distance between Bet/raise button and others
...
#############  GUI ends HERE #####################
    ...
    def clearActivePlayer(self):
        for label in self.labelp2labelList:
            label['bg'] = 'grey'
    ...
    def setActivePlayerFlag(self,playerMappingList,activePlayerList):
        ...
        for label,count in zip(self.labelp2labelList, range(len(self.labelp2labelList))):
            if count == index:
                label['bg'] ='ivory' '''
    ...
    def printTable(self):
        ...
        if platform.system() == "Windows": #Unicode works won windows 10
            ...
            self.display.myCards = replaceMac1(self.display.myCards)
        #debug
        #self.labelp6['text']:'newRequest \n' + str(self.display.newRequest) +'\n' + str(self.display.phase)
               
        #setting layout here
        #clearActivePlayer(self) #clears all the colors on player labels
        playerMappingList = []
